...
@Component
public class QuestionWordsImpl implements WorldService {
    @Autowired
    Data data;
...
//                    +  getIsQustion()
//                    ;
        return getQustionWorld() + getPronoun() + "  " + world + " " + getTense() + "  "
//                + getYesOrNo()
                + getIsQustion();
//                +getContinuousOrUsually();
//                + (isNo ? "" : getIsQustion());

    }

    private String getQustionWorld() {
        Map<Integer, String> map = Map.of(0, "Кто", 1, "Что", 2, "Какой", 3, "Когда", 4, "Куда", 5, "Откуда", 6, "Почему", 7, "Зачем", 8, "Как",  9,"Как часто");
        Map<Integer, String> map2=new HashMap<>(map);
        map2.put(10,"Сколько");
        return map.get(getRandom(11))+" ";
    }
